...
concepts_text = {
    ...
    "d": """<p><strong>Effect Size (Cohen's d)</strong>: A standardized measure of the difference in the means (can be sample or population means depending on the context). The difference in means is divided by the <a href="https://en.wikipedia.org/wiki/Pooled_variance" target="_blank">pooled standard deviation</a> of the two samples/populations to provide a metric (in standard deviations) that can be compared across studies. It can also be used directly in some calculations instead of the means and standard deviations.</p>"""
}
